 ...
 */
byte MFRC522::
PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
...
string MFRC522::PICC_ConvertToUpperCase(const string &inputString) {
...
string MFRC522::PICC_TakeStringID() {
  stringstream ID_string;
    for(byte i = 0; i < uid.size; ++i)
    {
      ...
      {
        ID_string << "0";
        ID_string << std::hex << static_cast<int>(uid.uidByte[i]);
      }
      else
      {
        ID_string << std::hex << static_cast<int>(uid.uidByte[i]);
    ...
    return PICC_ConvertToUpperCase(ID_string.str());
}