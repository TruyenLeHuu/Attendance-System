...
class Point_ft
{
    ...
    public:
        Point_ft(int id, vector<float> landmark)
        {
            pointId = id;
            values = landmark;
...
private:
    ...
    vector<Point_ft> points;
    // cv::Mat feature__;

public:
    Cluster(int clusterId, Point_ft centroid)
    {
    ...
    Point_ft getPoint(int pos) { return points[pos]; }

    int getSize() { return points.size(); }

    double getCentroidByPos(int pos) { return centroid[pos]; }

    cv::Mat getCentroid() { return cv::Mat(centroid,true);}
...
private:
    ...
    int getNearestClusterId(Point_ft point)
    {
...
public:
    ...
    vector<cv::Mat> run(vector<Point_ft> &all_points)
    {
        ...
        // ofstream outfile;
        // cerr<<output_dir + "/" + to_string(K) + "-clusters.txt"<<endl;
